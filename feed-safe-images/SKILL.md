...
# horizontal photo that would lose its edges to the film crop: black canvas, rounded corners
scripts/pad.sh igblack IMG_005.jpg
```
...
`film` cover-crops, so a 16:9 photo loses about 40% of its width and anything near the left or right edge is gone. When that matters, use `igblack` instead: the `ig` canvas in black with the photo corners rounded (`--radius 91`, the frame's corner radius scaled to the canvas), whole photo visible, same dark look in the feed. `--radius` works on any padded layout, and `--bg` still overrides the canvas color.
...
1. Ask how many photos go in the post and which platform. Pick the layout from the table. One layout covers single posts and multi-image posts on every platform; for X that is `x1` for a single image and `xc` for every slide of a 2-4 image post. For Instagram, `film` is the alternative look when the user asks for a film, vintage, or bordered frame; if the photo is horizontal and the edges matter, `igblack` keeps the whole frame.
...
## Hosting for Instagram's API

Instagram's Graph API has no upload endpoint: `create_media_container` takes an `image_url`, and Meta fetches it once. `scripts/host.sh <file>...` uploads each file to a Cloudflare R2 bucket and prints one public URL per file, in order. Setup once: `wrangler login`, create a bucket with a public dev URL and a 1-day expiry lifecycle rule (`wrangler r2 bucket create NAME`, `wrangler r2 bucket lifecycle add NAME --name expire-1d --expire-days 1`, `wrangler r2 bucket dev-url enable NAME`), then write `~/.config/cloudflare/r2-staging.env` with `R2_STAGING_BUCKET` and `R2_STAGING_PUBLIC_URL`. Keep that file out of any repo. If `*.r2.dev` is blocked on your network, verify by creating the Instagram container and polling its status to `FINISHED`, not with curl.
